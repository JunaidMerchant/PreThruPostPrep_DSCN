...
#
#
# The following lines of code will run freesurfer before running fmriprep, which considerably speeds up time.
# This requires using the -parallel option to run a lot of the processes in parallel. Second, it requires 
# you make sure that the outputted freesurfer data is where fmriprep will look for it. 
...
echo "------------------------------------------------------------------"
#
recon-all -i /data/bswift-1/jmerch/JAM/in/${Sub}/anat/${Sub}_T1w.nii.gz -openmp 6 -s ${Sub} \
-sd /data/bswift-1/jmerch/JAM/out/freesurfer -all -parallel
#
#
echo "------------------------------------------------------------------"
...
# Depending on if you have fieldmap scans or not, you may want to choose the appropriate options from fmriprep. 
#
#
